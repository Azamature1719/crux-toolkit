...
TideSearchApplication::TideSearchApplication() : exact_pval_search_(false), remove_index_(""), spectrum_flag_(NULL)
{
}
...
vector<InputFile> TideSearchApplication::getInputFiles(
    const vector<string> &filepaths) const
{
...
void TideSearchApplication::search(void *threadarg)
{
       ...
       sc = sc + num_threads)
  {
    ...
    if (curScoreFunction == XCORR_SCORE && !exact_pval_search_)
    { // execute original tide-search program
      ...
      *total_candidate_peptides += nCandPeptide;
      locks_array[LOCK_CANDIDATES]->unlock();
        ...
        unsigned int size_cache = MaxBin::Global().CacheBinEnd() * NUM_PEAK_TYPES; 

        int score_result = 0;
        score_result = active_peptide_queue->GpuBasedScore(cache, size_cache);
    ...
    { // This runs curScoreFunction=BOTH_SCORE, curScoreFunction=RESIUDUE_EVIDENCE_MATRIX, and xcorr p-val
      ...
      *total_candidate_peptides += (long)nCandPeptide;
      ...
      for (peidx = 0; peidx < candidatePeptideStatusSize; peidx++)
      {
        if ((*candidatePeptideStatus)[peidx])
        {
          ...
          if (curScoreFunction != XCORR_SCORE)
          {
            ...
            { // if > 0, set bool to true to create DP matrix
      ...
      if (curScoreFunction != RESIDUE_EVIDENCE_MATRIX)
      {
        ...
        for (pe = 0; pe < nPepMassIntUniq; pe++)
        {                                      // TODO should probably instead use iterator over pepMassIntUnique
          int pepMaInt = pepMassIntUnique[pe]; // TODO should be accessed with an iterator
      ...
      if (curScoreFunction != XCORR_SCORE)
      {
        for (pe = 0; pe < nPepMassIntUniq; pe++)
        {
          ...
          int maxScore = 0;
          for (int i = 0; i < maxNResidue; i++)
          { // maxColEvidence has been sorted
...
{
  return "[[nohtml:Search a collection of spectra against a sequence database, "
    ...
    double *pValueScoreObs)
{
  ...
  int colLast = MassConstants::mass2bin(MassConstants::bin2mass(pepMassInt) - MassConstants::mono_oh);
...
)
{
  ...
  for (de = 0; de < nAa; de++)
  {
    ...
    if (col <= maxAaMass + colLast && col >= initCountCol)
    { // TODO not sure if below or above is correct
